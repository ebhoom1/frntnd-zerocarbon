  ...
  const handleSubmit = async (e) => {
    ...
    cleanedFormData.activities.forEach((activity, activityIndex) => {
      ...
      activity.fuels = activity.fuels.filter((fuel, fuelIndex) => {
        const isValidFuel = fuel.name.trim();
        if (!isValidFuel) {
        ...
        fuel.units = fuel.units.filter((unit, unitIndex) => {
          const isValidUnit = unit.type.trim() && unit.kgCO2e.trim();
          if (!isValidUnit) {
            hasEmptyFields = true; // Track empty fields
          }
    ...
    if (hasEmptyFields) {
      const proceed = window.confirm(
        "Some fuels or units are incomplete or missing. Do you want to proceed with the submission?"
      );
      if (!proceed) {
    ...
    // Proceed with submitting cleaned data
    try {
      const response = await axios.post("/api/categories", cleanedFormData);
      alert("Emission Factor Created Successfully!");
      console.log(response.data);
    ...
    >
      ...
      <form onSubmit={handleSubmit}>
        <Grid container spacing={3}>
          ...
          {formData.activities.map((activity, activityIndex) => (
            <Grid item xs={12} key={activityIndex}>
              <Typography
                variant="h6"
                sx={{ borderBottom: "1px solid #ccc", paddingBottom: "4px" }}
              >
        ...
        <Button
          ...
          sx={{ mt: 4, ml: 4 }}
          onClick={() => {
            navigate("/adminemissionfactor-table");
          }}
        >
          View Table
        </Button>
