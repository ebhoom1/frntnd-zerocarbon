...
function PurchasedGoodsServices() {
  // -------------------------
  // State Variables
  // -------------------------
  ...
  const fetchRecords = async () => {
    try {
      const res = await api.get("/api/purchasedgoodsservices/get");
      setRecords(res.data);
    } catch (err) {
  ...
  const handleSave = async () => {
    try {
      if (editMode) {
        await api.put(
          `/api/purchasedgoodsservices/update/${selectedRecordId}`,
          formData
        );
      } else {
        const res=await api.post("/api/purchasedgoodsservices/add", formData);
  ...
  const handleDelete = async (id) => {
    try {
      await api.delete(`/api/purchasedgoodsservices/delete/${id}`);
      fetchRecords();
    } catch (error) {
  ...
  return (
    <>
      <AppBar position="static">
        <Toolbar>
          <Typography variant="h6">Purchased Goods & Services</Typography>
        </Toolbar>
      </AppBar>
