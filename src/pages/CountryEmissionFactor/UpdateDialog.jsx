...
const UpdateDialog = ({ open, onClose, editData, setEditData, fetchEmissionFactors }) => {
  ...
  const addYearlyValueField = () => {
    setEditData({
      ...editData,
      yearlyValues: [...editData.yearlyValues, { from: '', to: '', value: '' }],
    });
  };
        ...
        {editData?.yearlyValues.map((yearlyValue, index) => (
          <Box key={index} sx={{ display: 'flex', gap: 2, alignItems: 'center', mb: 2 }}>
            <TextField
              ...
              onChange={(e) => handleYearlyValuesChange(index, 'from', e.target.value)}
              required
            />
            <TextField
              label="To (dd/mm/yyyy)"
              value={yearlyValue.to}
              onChange={(e) => handleYearlyValuesChange(index, 'to', e.target.value)}
              required
            />
            <TextField
              label="Value"
