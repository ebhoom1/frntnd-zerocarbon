...
//   Grid,
// } from "@mui/material";
// import EmissionsSimulationDisplay from './AdminPage/EmissionsSimulationDisplay'
...
const EmissionsReductionSimulator = () => {
  ...
  const resetForm = () => {
    ...
    setPolicyChanges("");
    setAiResponse(null);
    setIsSaved(false); // allow user to simulate again
  };
      ...
      <Box sx={{ p: 4 }}>
        <Container maxWidth="lg">
          <Paper elevation={4} sx={{ p: 4, textAlign: "center", backgroundColor: "#FFFFFF", borderRadius: "12px" }}>
            <Typography sx={{ fontSize: "2rem", fontWeight: "bold", color: "#388E3C" }}>
              Emissions Reduction Simulator
            </Typography>
            <Typography sx={{ fontSize: "1rem", color: "#666" }}>
              Adjust strategies and assess the impact on CO2e reduction and cost savings.
            </Typography>
          </Paper>

          <Grid container spacing={4} sx={{ mt: 4, alignItems: "stretch" }}>
            <Grid item xs={12} md={6}>
              <Paper elevation={4} sx={{ p: 4, backgroundColor: "#FFFFFF", borderRadius: "12px", height: "100%" }}>
                <Typography sx={{ fontSize: "1.2rem", color: "#666", mb: 2 }}>
                  Input Your Data
                </Typography>

                <TextField label="Strategy Adjustments" fullWidth value={strategyAdjustments} onChange={(e) => setStrategyAdjustments(e.target.value)} sx={{ mb: 2 }} />
                <TextField label="Annual Energy Consumption (kWh or CO₂e/kWh)" type="number" fullWidth value={energyConsumption} onChange={(e) => setEnergyConsumption(e.target.value)} sx={{ mb: 2 }} />
                <TextField label="Fuel Usage (Liters Petrol/Diesel/Natural Gas consumed annually)" type="number" fullWidth value={fuelUsage} onChange={(e) => setFuelUsage(e.target.value)} sx={{ mb: 2 }} />
                <TextField label="Building Efficiency Data" fullWidth value={buildingEfficiency} onChange={(e) => setBuildingEfficiency(e.target.value)} sx={{ mb: 2 }} />
                <TextField label="Vehicle Fleet Data" fullWidth value={vehicleFleet} onChange={(e) => setVehicleFleet(e.target.value)} sx={{ mb: 2 }} />
                <TextField label="Waste Management Practices" fullWidth value={wasteManagement} onChange={(e) => setWasteManagement(e.target.value)} sx={{ mb: 2 }} />
                <TextField label="Carbon Capture/Offset Programs" type="number" fullWidth value={carbonCapture} onChange={(e) => setCarbonCapture(e.target.value)} sx={{ mb: 2 }} />
                <TextField label="Financial Constraints (Budget)" type="number" fullWidth value={financialConstraints} onChange={(e) => setFinancialConstraints(e.target.value)} sx={{ mb: 2 }} />
                <TextField label="Expected Policy Changes" fullWidth value={policyChanges} onChange={(e) => setPolicyChanges(e.target.value)} sx={{ mb: 3 }} />

                <Button variant="contained" color="primary" fullWidth onClick={fetchSimulationData} disabled={loading}>
                  {loading ? <CircularProgress size={24} sx={{ color: "white" }} /> : "Simulate Impact"}
                </Button>
  ...
  disabled={isSaved}
>
  {isSaved ? "Already Saved" : "Save Simulation"}
</Button>
                 ...
                 <Button
                      ...
                      variant="contained"
                      fullWidth
                      onClick={() => setShowSavedResponses((prev) => !prev)}
                    >
                      {showSavedResponses ? "Hide Saved Responses" : "View Saved Responses"}
                    </Button>
              </Paper>
            </Grid>
            ...
            <Grid item xs={12} md={6}>
              {aiResponse ? (
                <>
                  <Paper elevation={4} sx={{ p: 4, textAlign: "center", backgroundColor: "#FFFFFF", borderRadius: "12px", height: "auto", position: "relative", overflow: "hidden" }}>
                    <Typography sx={{ fontSize: "1.2rem", fontWeight: "bold", color: "#4CAF50", mt: 3 }}>
                      🌍 Estimated CO₂ Reduction: <strong>{aiResponse?.reduction ?? "N/A"}</strong> tonnes
                    </Typography>
                    <Typography sx={{ fontSize: "1.2rem", fontWeight: "bold", color: "#0288D1", mt: 2 }}>
                      💰 Cost Savings: <strong>${aiResponse?.savings?.toLocaleString() ?? "N/A"}</strong>
                    </Typography>
                    <Typography sx={{ fontSize: "1.2rem", fontWeight: "bold", color: "#D32F2F", mt: 2 }}>
                      🏗️ Investment Needed: <strong>${aiResponse?.investment?.toLocaleString() ?? "N/A"}</strong>
                    </Typography>
                  </Paper>

                  <Paper elevation={4} sx={{ p: 2, textAlign: "center", backgroundColor: "#FFFFFF", borderRadius: "12px", mt: 2, minHeight: "300px" }}>
                    <Typography sx={{ fontSize: "1.5rem", fontWeight: "bold", color: "#388E3C" }}>
                      Cost-Benefit Analysis
                    </Typography>
                    <ResponsiveContainer width="100%" height={250}>
                      <BarChart data={[
                        { category: "Savings", value: aiResponse.savings ?? 0 },
                        { category: "Investment", value: aiResponse.investment ?? 0 },
                ...
                <Typography sx={{ fontSize: "1rem", fontWeight: "bold", color: "#777", mt: 3 }}>
                  🔍 Enter your data to generate insights and simulate impact.
                </Typography>
              )}
            </Grid>
          ...
          {/* View Saved Simulations Section */}
          {showSavedResponses && <EmissionsSimulationDisplay userId={userId} />}
        </Container>
      </Box>
    </ThemeProvider>
  );
